...
7. **保存模型文件**。
    ::: details
    点击 BlockBench 左上角菜单：*文件 → 保存项目*（如下图），或按下 `Ctrl`+`Alt`+`S` 快捷键。
...
12. 点击“确定”按钮，显示的界面如下图所示。
    <Image src="https://s2.loli.net/2024/07/20/GWNdFBuixZaUf9O.png" />
...
13. 在界面左侧的搜索框中，输入参考方块的名称，例如 `LCD_L`，如下图。
    <Image src="https://s2.loli.net/2024/07/20/XW5DsRE3ragvZT7.png" />
    双击搜索出来的方块，弹出“选定 LCD 参考方块”对话框，如下图。
    ::: tip
    双击时光标应处于方块名称上，处于名称外无法触发。右键菜单同理。
    :::
    <Image src="https://s2.loli.net/2024/07/20/RScjYKZni7Q5xf8.png" />
    其中：
...
14. 点击“确定”按钮，显示的界面如下图所示。
    <Image src="https://s2.loli.net/2024/07/20/wRt5XoAzaNSjKEq.png" />
    ::: danger 遇到错误？
...
15. 在界面右上角，点击“管理同侧 LCD…”按钮，弹出“配置同侧 LCD”对话框，如下图。
    ...
    <Image src="https://s2.loli.net/2024/07/20/juqvspRKgF1HayI.png" />
    :::
16. 点击“确定”按钮，“进行配置”文本域中会显示更新的 `offsets` 信息。点击下面的“生成配置(G)”按钮，在“获取结果”文本域中将生成该模型的槽位配置。点击“复制结果(C)”按钮。
    ::: danger 遇到错误？
    ...
    :::
17. 将复制的槽位配置粘贴入第 4 步创建的 `slots.json` 中即可。
...
也可点击“调整旋转角度…”按钮，把一些诸如 ±180°、±360° 的角度改成 0°（如果有）。重新生成配置，看看能否解决问题。

如果仍无法解决问题，如不介意，可将有问题的资源包发送至 [jeffreyg1228@126.com](mailto:jeffreyg1228@126.com)，如果我有空可能会帮助你解决一下问题。注意标注好参考方块的名称。
...
2. 重新生成配置并编辑 `slots.json`，然后压缩并替换原来的资源包，进入游戏，按 `F3`+`T` 快捷键重载资源包。
